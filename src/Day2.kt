fun main() {
    ...
    assert(run("1,0,0,0,99").instructions == "2,0,0,0,99")
    assert(run("2,3,0,3,99").instructions == "2,3,0,6,99")
    assert(run("2,4,4,5,99,0").instructions == "2,4,4,5,99,9801")
    assert(run("1,1,1,4,99,5,6,0,99").instructions == "30,1,1,4,2,5,6,0,99")
...
fun findNounAndVerb(program: String, target: Int): Int {
    for (noun in (0..1000)) {
        for (verb in (0..1000)) {
            try {
                if(run("1,$noun,$verb,${program.drop(6)}").instructions.split(",").first().toInt() == target) {

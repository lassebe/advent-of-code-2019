...
fun minimumDistance(locations: Set<Pair<Int, Int>>): Int = locations.map { abs(it.first) + abs(it.second) }.min()!!

fun intersections(firstWire: String, secondWire: String): Set<Pair<Int, Int>> =
        locations(firstWire)
                .intersect(
                        locations(secondWire)
                )

fun locations(wire: String): Set<Pair<Int, Int>> {
    val origin = Pair(0, 0)
    var start = origin
    val wireLocations = mutableListOf<Pair<Int, Int>>()
...
fun move(start: Pair<Int, Int>, direction: String, steps: Int) = when (direction) {
    "R" -> (start.first..start.first + steps).map { Pair(it, start.second) }
    "L" -> (start.first - steps..start.first).map { Pair(it, start.second) }.asReversed()
    "U" -> (start.second..start.second + steps).map { Pair(start.first, it) }
    "D" -> (start.second - steps..start.second).map { Pair(start.first, it) }.asReversed()
